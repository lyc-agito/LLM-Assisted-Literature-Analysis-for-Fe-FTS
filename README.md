...
***
## file_processor.py
- `file_processor.py`定义了函数`*list_to_txt`、`data_to_json`、`find_txts`、`*rename_txts`、`*json_excel`、`*answer_split`、`*poly_input`，用于实现基本的文件读写等操作；
- 2
- `file_processor.py` defines functions such as `list_to_txt`, `data_to_json`, `find_txts`, `rename_txts`, `json_excel`, `answer_split`, and `poly_input` to handle basic file operations like reading and writing.
- One should place the `file_processor.py` file within your Python project directory for direct import.
## api_MinerU.py
...
## original.py、related.py、loading.py、size_max.py
